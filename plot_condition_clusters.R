...
preview_png <- function(path, title = NULL) {
  if (!interactive()) {
    ...
    if (capabilities("aqua")) {
      try(
        quartz(
          width = plot_width,
          height = plot_height,
          title = if (is.null(title)) basename(path) else title
        ),
        silent = TRUE
      )
    }
  ...
  if (dev.cur() == 1) {
    warning(sprintf("Could not open a graphics device to preview %s.", basename(path)))
    return(invisible(NULL))
  }
...
dm1_path <- file.path(project_dir, "new_coll_dm1_participants_csm_rick.png")
dm2_path <- file.path(project_dir, "new_coll_dm2_participants_csm_rick.png")
dm3_path <- file.path(project_dir, "new_coll_dm3_participants_csm_rick.png")

save_plot_png(plot_dm1, dm1_path)
save_plot_png(plot_dm2, dm2_path)
save_plot_png(plot_dm3, dm3_path)

preview_png(dm1_path, title = "1 Difference-maker")
preview_png(dm2_path, title = "2 Difference-makers")
preview_png(dm3_path, title = "3 Difference-makers")
